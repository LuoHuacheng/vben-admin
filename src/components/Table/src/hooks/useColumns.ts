...
export function useColumns(
  ...
  const getColumnsRef = computed(() => {
    ...
    columns.forEach((item) => {
      const { customRender, customSlots } = item;

      handleItem(
        item,
        Reflect.has(item, 'ellipsis')
          ? !!item.ellipsis
          : !!ellipsis && !customRender && !customSlots,
      );
    });
    return columns;
  });
  ...
  const getViewColumns = computed(() => {
    ...
    return columns
      ...
      .map((column) => {
        const { customSlots, customRender, format, edit, editRow, flag } = column;

        if (!customSlots || !customSlots?.title) {
